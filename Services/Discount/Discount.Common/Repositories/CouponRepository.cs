    ...
    internal class CouponRepository : ICouponRepository
    {
        ...
        public async Task<bool> CreateDiscount(CreateCouponDTO couponDTO)
        {
            ...
            var affected = await connection.ExecuteAsync(
                "INSERT INTO Coupon (ProductName, Description, Amount) VALUES (@ProductName, @Description, @Amount)", new { ProductName = couponDTO.ProductName, Description = couponDTO.Description, Amount = couponDTO.Amount });
            return affected != 0;
        }
        ...
        public async Task<bool> UpdateDiscount(UpdateCouponDTO couponDTO)
        {
            using var connection = _context.GetConnection();
            var affected = await connection.ExecuteAsync(
                "UPDATE Coupon SET ProductName=@ProductName, Description=@Description, Amount=@Amount WHERE Id=@Id", new { couponDTO.ProductName, couponDTO.Description, couponDTO.Amount, couponDTO.Id});
            return affected != 0;
        }
        ...
        public async Task<IEnumerable<CouponDTO>> GetRandomDiscounts(int numberOfDiscounts)
        {
            ...
            var r = new Random();
            return _mapper.Map < IEnumerable < CouponDTO >>(allCoupons.Select(c => new { Number = r.Next(), Item = c })
